...
void printw(char* val) {  // implements printf i. i put as input a string
  write(1, val,
        strlen(val));  // In write for first place we can either use a file
                       // descriptor obtained from the open system call, or you
                       // can use 0, 1, or 2, to refer to standard input,
                       // standard output, or standard error, respectively.we
                       // print val that has length strlen(val)
}

int files_exist() {
  if ((access(DATAFILE, F_OK) == -1) &&
      (access(POSFILE, F_OK) ==
       -1)) {  // checks if both files exist. Access() checks whether the
               // calling process can access the file pathname. F_OK tests
               // whether the file exists. returns -1 if file doesnt exist
...
void flip(char* str) {  // flips name and surname for one entry only
  ...
  for (stop = 0; str[stop]; stop++) {  // till str has characters
    if (str[stop] == sep &&
        selector == 0) {  // if we meet space and selector is 0- do exmample to
                          // undestnd better
      selector = 1;
      first = stop + 1;
      continue;
    }
    fs[selector][stop - first] = str[stop];  // puts name and surname into fs[1]
                                             // and fs[0] so seperates them
  }
  ...
  for (int j = 0; fs[0][j];
       j++) {  // from where we were after we put surname and space we put name
    str[pos + j] = fs[0][j];
  }
}

void flip_array(char arr[][128],
                int arrsize) {         // flips many times for all entries
...
void load_data(char arr[][BSIZE]) {  // loads data in memory
  if (!files_exist())
    return;  // checks if files exist
  int anum2;
  int tanum = 0;
  int fd1 =
      open(DATAFILE, O_RDONLY);  // open the data.bin file in order to be read
  int nfile2 =
      open(POSFILE, O_RDONLY);  // open the pos.bin file in order to be read
  int i = 0;
  while (read(
      nfile2, &anum2,
      sizeof(
          int))) {  // read size of int bytes from nfile2 and put them in anum2
    lseek(nfile2, 0, SEEK_CUR);  // makes sure we continue reading the file's
                                 // bytes from where we where last time
    tanum += anum2;              // total number of bytes read
    memset(arr[i], '\0', BSIZE * sizeof(char));  // clear buffer
    read(fd1, arr[i],
         anum2);  // read anum2 bytes from fd1 and put them in arr[i]
...
void save2file(char arr[][BSIZE], int arrsize) {  // saves entries to files
  int srs = open(
      DATAFILE, O_WRONLY | O_CREAT | O_TRUNC,
      0644);  // open datafile. o_trunk=if there is anything in there clear them
...
int get_number_of_records() {  // gets and returns number of records
  if (!files_exist())
    return 0;  // checks if files exist
  int anum;
  unsigned int counter = 0;
  int nfile = open(POSFILE, O_RDONLY);  // opens pos.bin file
  while (read(nfile, &anum,
              sizeof(int))) {  // read sizeof(int) bytes from nfile and put them
                               // in anum. we are just using it there to count
    counter++;
  }
  close(nfile);
  return counter;
...
static int cmp_str(const void* lhs,
                   const void* rhs) {  // used only by qshort-callback function
...
int insert() {  // inserts entry
  ...
  int fd1 = open(DATAFILE, O_WRONLY | O_CREAT,
                 0644);  // open the file DATAFILE if not existing create it
  int nfile = open(POSFILE, O_WRONLY | O_CREAT,
                   0644);  // open the file POSFILE if not existing create it
  ...
  write(fd1, entry, entry_size);  // read entry_size of bytes from DATAFILE and
                                  // put them in entry
  write(
      nfile, &entry_size,
      sizeof(int));  // read sizeof(int) from POSFILE and put them in entry size
  close(fd1);
  close(nfile);
  sort_list();  // sort
...
void delete () {
  ...
  if (todelete >
      counter) {  // if the choise you made is larger that record number
  ...
  int srs = open(DATAFILE, O_WRONLY | O_CREAT | O_TRUNC,
                 0644);  // opens datafile to write without the user selected
                         // entry deletes previous one
...
void edit() {  // edits the entry the user chooses
  ...
  if (toedit > counter) {  // if record you choose to edit is greater than the
                           // number of records
    printf("Number (%d) exceeds max record number (%d)\n", toedit, counter);
    return;
  }
...
void display_all() {
  ...
  int tanum = 0;
  int fd1 =
      open(DATAFILE, O_RDONLY);  // file descriptor that shows that file opened
  ...
  while (read(
      nfile, &anum,
      sizeof(
          int))) {  // read the file that file decriptor shows,read sizeof(int)
                    // bytes a time and put them into the adress of anum
    lseek(nfile, 0,
          SEEK_CUR);  // leaves us at the position we were the last time we read
    printf("%d.", record_no++);
    memset(buffer, '\0', BSIZE * sizeof(char));
    tanum += anum;            // total bytes read
    read(fd1, buffer, anum);  // read the file that fd1 describes, read anum
                              // bytes a time and put them in buffer
    printf("%s\n", buffer);  // print the bytes that are in the buffer
    lseek(fd1, tanum,
          SEEK_SET);  // starts from the start (SEEK_SET) t and goes to tanum
  }
  close(nfile);
  close(fd1);
  printw("\n");
  ...
  while (read(
      ...
      sizeof(
          int))) {  // read the file that file decriptor shows,read sizeof(int)
                    // bytes a time and put them into the adress of anum//
    lseek(nfile, 0,
          SEEK_CUR);  // leaves us at he position we were the last time we read
    ...
    lseek(fd1, tanum,
          SEEK_SET);  // starts from the start (SEEK_SET) t and goes to tantum
  }
  close(nfile);
  close(fd1);
  if (!found) {
